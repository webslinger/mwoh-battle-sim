...
define(["./encounter","./session", "./deck", "./presets"], function (encounter, session, deck, presets) {
	...
	function Simulator(cards, offense, defense, support_o, support_d) {
		console.log(arguments);
		console.log(support_o);
		this.cards = cards;
		this.offenseSession = session.create();
		this.defenseSession = session.create();
		this.offense = deck.create("offense", offense, this.cards[0], ["offense", support_o, this.cards[0]]);
		this.defense = deck.create("defense", defense, this.cards[0], ["defense", support_d, this.cards[0]]);
		this.fixed_ob = 3;
		this.fixed_db = 3;
		for (var i = 0; i < 3; i++) {
	...
	Simulator.prototype = {
		...
		battle: function(blocking, is_fixed) {
			var fixed_o = encounter.create(), fixed_d = encounter.create(),
				...
				off_support = false, def_support = false;
			...
			// support deck
			if (this.offense.support_enabled) {
				ability = this.offense.supportAffect(this.offense, this.defense, is_fixed);
				off_boost = this.offense.support.supportBoost("atk");
				fixed_o.supportAbility = ability;
				fixed_o.supportBoost = off_boost;
			}
			
			if (this.defense.support_enabled) {
				ability = this.defense.supportAffect(this.defense, this.offense, is_fixed);
				def_boost = this.defense.support.supportBoost("def");
				fixed_d.supportAbility = ability;
				fixed_d.supportBoost = def_boost;	
			}
			
			// totals
			fixed_o.total = this.offense.total() + parseInt(off_boost);
			fixed_d.total = this.defense.total() + parseInt(def_boost);
		...
		changeCard: function(view, data) {
			...
			if (view.length < 4) {
				...
				for (var a = 0; a < this.offense.support.cards.length; a++) {
					if (this.offense.support.cards[a].view === view) {
						found = true;
						return this.offense.support.cards[a].update(data);
					}
				}
				if (!found) {
					for (var b = 0; b < this.defense.support.cards.length; b++) {
						if (this.defense.support.cards[b].view === view) {
							return this.defense.support.cards[b].update(data);	
						}
					}
				}
			}
		...
		supportToggle: function(view) {
			if (view[0] === "o") {
				this.offense.support_enabled = !this.offense.support_enabled;
			} else {
				this.defense.support_enabled = !this.defense.support_enabled;
			}	
		},
	...
	return {
		initialize: function(cards, offense, defense, o_support, d_support) {
			return new Simulator(cards, offense, defense, o_support, d_support);
		}
	}
});