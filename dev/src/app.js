...
function ($, model) {
	...
	function initialize(input) {
		...
		$(".card, .support-deck select").not(".support-skill").each(function() {
		...
		$("#off-combo, #def-combo, #off-support, #def-support").prop("checked", true);
		...
		simulator = model.initialize(output, ["oc1","oc2","oc3","oc4","oc5"], ["dc1","dc2","dc3","dc4","dc5"], ["off-support1", "off-support2", "off-support3"], ["def-support1", "def-support2", "def-support3"]);
		...
		$(".support-deck").on("change", function(e) {
			var view = $(e.target).attr("id");
			simulator.changeCard(view, $(e.target).find("option:selected").attr("id"));
			battleLog(simulator.battle(false, true));
		});
		...
		$(".deck-options").on("change", function(e) {
			switch ($(e.target).attr("id")) {
				...
				case "off-support":
				case "def-support": simulator.supportToggle($(e.target).attr("id")); break;
			}
			battleLog(simulator.battle(false, true));
		});
	...
	function battleLog(results) {
		...
		if (offense.supportAbility) {
			off_log += "<span>"+offense.supportAbility[0]+((offense.supportAbility[1]==="Degrade")?" support degraded ":" support boosted ")+offense.supportAbility[2]+": <span>"+offense.supportAbility[3]+"</span></span>";	
		}
		
		if (offense.supportBoost) {
			off_log += "<span>Support Deck Boosted All: <span>"+offense.supportBoost+"</span></span>";	
		}
		...
		if (defense.supportAbility) {
			def_log += "<span>"+defense.supportAbility[0]+((defense.supportAbility[1]==="Degrade")?" support degraded ":" support boosted ")+defense.supportAbility[2]+": <span>"+defense.supportAbility[3]+"</span></span>";	
		}
		
		if (defense.supportBoost) {
			def_log += "<span>Support Deck Boosted All: <span>"+defense.supportBoost+"</span></span>";	
		}
